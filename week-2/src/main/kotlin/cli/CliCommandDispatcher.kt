...
class CliCommandDispatcher(
    ...
    suspend fun handle(command: Command, state: CliState): CliState = when (command) {
        ...
        is Command.Back -> handlers.todoTask.handleBack(state) { renderer.renderMenu(handlers.command.getAllExecutors()) }
        ...
        is Command.AddFact,
        is Command.RemoveFact -> state.also { renderer.renderInfo("Команды стратегий будут доступны в Task 5") }
    ...
    private fun handleSelectTask(command: Command.SelectTask, state: CliState): CliState {
        ...
        val executor = handlers.command.getExecutor(TaskId(command.taskId.toString()))
        if (executor != null) {
