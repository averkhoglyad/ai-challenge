package io.averkhogliad.ai.challenge.week4.cli.infrastructure.indexer.chunker
...
class StructuralChunker(
    ...
    private fun chunkByHeadings(
        ...
        for ((heading, sectionText) in sections) {
            ...
            if (trimmed.length > maxSectionLength) {
                // Разбиваем длинную секцию на под-чанки
                val subChunks = splitLongSection(trimmed, maxSectionLength)
                subChunks.forEach { subText ->
                    chunks.add(
                        buildChunk(subText, source, title, heading, runId)
                    )
                }
            } else {
    ...
    /**
     * Разбивает Plain Text / HTML по абзацам, объединяя смежные короткие.
     */
    ...
    /**
     * Разбивает длинный текст на под-чанки равного размера.
     */
