...
object CommandParser {
    ...
    internal fun parseCommandWithArgs(commandName: String, args: String, raw: String): Command {
        return when (commandName) {
            ...
            "index" -> parseIndexCommand(args, raw)
            "index-runs" -> Command.IndexRuns
            "index-switch" -> parseIndexSwitchCommand(args, raw)
            "index-stats" -> parseIndexStatsCommand(args)
    ...
    // ═══════════════════════════════════════════════════════════════
    // Парсинг команд индексации документов
    // ═══════════════════════════════════════════════════════════════

    /**
     * Парсит команду индексации: `:index <strategy> <path> [--chunk-size N] [--overlap N] [--batch-size N]`.
     * strategy: "fixed" или "structural", path: путь к файлу или директории.
     */
    internal fun parseIndexCommand(args: String, raw: String): Command {
        val tokens = args.split(" ")
        if (tokens.size < 2) return Command.Unknown(raw)

        val strategy = tokens[0]
        val path = tokens[1]

        return Command.Index(strategy, path)
    }

    /**
     * Парсит команду переключения активного индекса: `:index-switch <runId>`.
     */
    internal fun parseIndexSwitchCommand(args: String, raw: String): Command {
        return if (args.isEmpty()) Command.Unknown(raw)
        else Command.IndexSwitch(args)
    }

    /**
     * Парсит команду показа статистики: `:index-stats [runId|all]`.
     */
    internal fun parseIndexStatsCommand(args: String): Command {
        return when {
            args == "all" || args.startsWith("all ") -> Command.IndexStats("all")
            args.isEmpty() -> Command.IndexStats(null)
            else -> Command.IndexStats(args)
        }
    }

    /**
     * Парсит команду сравнения индексов: `:index-compare <runId1> <runId2>`.
     */
    internal fun parseIndexCompareCommand(args: String, raw: String): Command {
        val parts = args.split(" ", limit = 2)
        if (parts.size < 2 || parts[0].isEmpty() || parts[1].isEmpty()) return Command.Unknown(raw)
        return Command.IndexCompare(parts[0], parts[1])
    }

    /**
     * Парсит команду удаления: `:index-delete <runId>`, `:index-delete --before DATE`, `:index-delete --keep-last N`.
     */
    internal fun parseIndexDeleteCommand(args: String, raw: String): Command {
        return when {
            args.startsWith("--before ") -> {
                val date = args.removePrefix("--before ").trim()
                if (date.isEmpty()) Command.Unknown(raw)
                else Command.IndexDeleteBefore(date)
            }

            args.startsWith("--keep-last ") -> {
                val count = args.removePrefix("--keep-last ").trim().toIntOrNull()
                if (count == null || count <= 0) Command.Unknown(raw)
                else Command.IndexDeleteKeepLast(count)
            }

            args.isEmpty() -> Command.Unknown(raw)
            else -> Command.IndexDelete(args)
        }
    }

    /**
     * Парсит команду очистки: `:index-clear` или `:index-clear --all`.
     */
