...
import io.averkhogliad.ai.challenge.week4.cli.infrastructure.persistence.SqliteMCPServerRepository
    ...
    "SqliteMCPServerRepository" - {
        ...
        "save and findById" - {
            ...
            "should return null when id does not exist" {
                runTest {
                    val found = repository.findById(ModelId("nonexistent-id"))
                    found shouldBe null
                }
            }
            ...
            "should throw exception when saving a config with a duplicate name" {
                runTest {
                    ...
                    shouldThrow<IllegalStateException> {
                        runTest { repository.save(config2) }
                    }

                    val found1 = repository.findById(config1.id)
                    found1 shouldNotBe null
                    found1!!.name shouldBe "duplicate-name"
                    found1.transport shouldBe MCPTransport.Stdio("echo")

                    val found2 = repository.findById(config2.id)
                    found2 shouldBe null
                }
            }
        }
        ...
        "findByName" - {
            "should find a server config by its name" {
                runTest {
                    ...
                    val found = repository.findByName("find-me")
                    found shouldNotBe null
            ...
            "should return null when name does not exist" {
                runTest {
                    val found = repository.findByName("no-such-server")
                    found shouldBe null
                }
            }
        }
        ...
        "delete" - {
            ...
            "should be idempotent when deleting a non-existent id" {
                runTest {
                    repository.delete(ModelId("not-there"))
                    // No exception expected — delete is idempotent, returns Unit
                }
            }
        }
        ...
        "existsByName" - {
            "should return true when the name exists" {
                runTest {
                    ...
                    val exists = repository.existsByName("existing")
            ...
            "should return false when the name does not exist" {
                runTest {
                    val exists = repository.existsByName("nope")
            ...
            "should return false when the repository is empty" {
                runTest {
                    val exists = repository.existsByName("anything")
                    exists shouldBe false
                }
            }
        }
