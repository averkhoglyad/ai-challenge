...
import io.averkhogliad.ai.challenge.week3.cli.cli.renderers.FsmRenderer
import io.averkhogliad.ai.challenge.week3.cli.cli.renderers.InvariantRenderer
import io.averkhogliad.ai.challenge.week3.cli.cli.renderers.ProfileRenderer
import io.averkhogliad.ai.challenge.week3.cli.cli.renderers.TaskRenderer
...
class ConsoleCliRenderer : CliRenderer {
    ...
    override fun renderHelp(state: CliState) {
        ...
        if (state.currentTaskId == null && !state.taskListMode) {
            ...
            println("  ${CYAN}:temp [value]${RESET}             — установить температуру (0.0-2.0); без аргументов — показать")
            println("  ${CYAN}:maxtokens [n]${RESET}            — установить макс. кол-во токенов; без аргументов — показать")
            println("  ${CYAN}:stop [s1,s2,...]${RESET}         — установить стоп-последовательности; без аргументов — сбросить")
    ...
    override fun renderWelcome() {
        println()
        println("${CYAN}👋 Добро пожаловать в AI Challenge!${RESET}")
        println("${CYAN}💡 Введите :help для справки.${RESET}")
        println()
    }
