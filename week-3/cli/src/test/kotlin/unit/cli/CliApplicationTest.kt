...
class CliApplicationTest : FreeSpec({
    ...
    val stubTaskRepository = object : io.averkhogliad.ai.challenge.week3.cli.domain.service.TaskRepository {
    ...
    val stubFactRepository = object : io.averkhogliad.ai.challenge.week3.cli.domain.service.FactRepository {
    ...
    val stubPromptBuilder = io.averkhogliad.ai.challenge.week3.cli.domain.service.PromptBuilder()

    val stubDialogService = io.averkhogliad.ai.challenge.week3.cli.application.DialogService(
        llmPort = null,
    ...
    val stubCommandEngine = io.averkhogliad.ai.challenge.week3.cli.application.DefaultCommandEngine()

    val stubDebugCommandHandler = DebugCommandHandler(
        DebugMode()
    )

    val stubPlanCommandHandler = io.averkhogliad.ai.challenge.week3.cli.application.handler.PlanCommandHandler(
        taskRepository = stubTaskRepository,
    ...
    "Command handling" - {
        ...
        "run uses shared fake input and real handler wiring" {
            ...
            renderer.renderedMessages.contains("menu:1") shouldBe true
            renderer.renderedMessages.contains("taskHeader:Mock Task 1") shouldBe true
        ...
        "status command routes through parser and dispatcher with active profile" {
            runTest {
                ...
                val profileService =
                    io.averkhogliad.ai.challenge.week3.cli.application.ProfileService(profileRepository)
        ...
        "todo detail user input uses dialog service task detail context before executor" {
            runTest {
                ...
                val memoryService =
                    io.averkhogliad.ai.challenge.week3.cli.domain.service.MemoryService(sessionRepository)
                ...
                executor.lastPrompt shouldBe null
                val detailSession = sessions["session_task_42"]
                (detailSession != null) shouldBe true
                detailSession!!.level shouldBe io.averkhogliad.ai.challenge.week3.cli.domain.model.SessionLevel.TASK_DETAIL
